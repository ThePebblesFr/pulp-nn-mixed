...
files = glob.glob('XpulpV2/*/src/Pooling/AvgPool/*.c') + glob.glob('XpulpV2/*/src/Pooling/MaxPool/*.c')
modified = []
for f in files:
    ...
    # 1) Add parameter to function signature for avgpool/maxpool if missing
    # Match "void [__attribute__ ...] pulp_nn_avgpool_* ( ... ) {" or "void pulp_nn_maxpool_* ( ... ) {"
    m = re.search(r'(void\s+(?:__attribute__\s*\(\(.*?\)\)\s*)?pulp_nn_(?:avgpool|maxpool)_[\w_]*\s*\()(.*?)(\)\s*\{)', s, re.S|re.I)
    if m:
        params = m.group(2)
    ...
    # 3) Insert modulo after core_id = pi_core_id(); for int/uint8_t
    ...
    while i < len(lines):
        ...
        if m2:
            # check next 3 lines for existing modulo
            next_has = False
            for j in range(1,4):
                if i + j < len(lines):
